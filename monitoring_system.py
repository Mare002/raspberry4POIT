...
def set_parameters():
    ...
    try:
        ...
        print(f"Nove prahy: Solar={solar_threshold}, Boiler={boiler_threshold}")
    ...
    while running:
        try:
            ...
            if line:
                parts = line.split(',')
                if len(parts) == 4:
                    solar, boiler, valve1, valve2 = float(parts[0]), float(parts[1]), int(parts[2]), int(parts[3])
...
def udaje():
    try:
        ...
        zaznamy = execute_db_query(query, fetch_all=True) or []
    except Exception as e:
        print("Chyba pri nacitani dat:", e)
        zaznamy = []
...
def api_ventily():
    try:
        ...
        print("Chyba pri načítaní ventilových dát:", e)
        return jsonify({})
...
def get_data_for_date(datum):
    try:
        ...
        query = "SELECT cas, teplota1, teplota2, ventil1, ventil2 FROM zaznamy WHERE cas BETWEEN %s AND %s ORDER BY cas"
...
def api_current_values():
    try:
        query = "SELECT teplota1, teplota2, ventil1, ventil2, cas FROM zaznamy ORDER BY id DESC LIMIT 1"
        ...
        if row:
            ...
            return jsonify({
                'teplota1': 0, 'teplota2': 0, 'ventil1': 0, 'ventil2': 0,
                'cas': '00:00:00', 'timestamp': datetime.now().isoformat()
            })
    except Exception as e:
        return jsonify({'error': 'Chyba pri načítaní dát'}), 500
...
def export_csv():
    ...
    try:
        ...
        for zaznam in zaznamy:
            writer.writerow([zaznam['cas'], zaznam['t1'], zaznam['t2'], zaznam['v1'], zaznam['v2']])
        ...
        return send_file(mem, as_attachment=True, download_name=f"teploty_{filename_date}.csv", mimetype='text/csv')
    except Exception as e:
...
def export_json():
    ...
    try:
        ...
        return send_file(mem, as_attachment=True, download_name=f"teploty_{filename_date}.json", mimetype='application/json')
    except Exception as e:
        return f"Chyba pri exporte: {str(e)}", 500
    ...
    try:
        ...
        for row in csv_input:
            if len(row) >= 5:
                ...
                teplota1, teplota2 = float(row[1]), float(row[2])
                ...
                query = "INSERT INTO zaznamy (cas, teplota1, teplota2, ventil1, ventil2) VALUES (%s, %s, %s, %s, %s)"
